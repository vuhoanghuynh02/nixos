# dnscrypt.nix
{
  ...
  config = {
    assertions = [
      {
        assertion = config.myOptions.dnscrypt.blocklist == '''' || config.services.dnscrypt-proxy2.enable;
        message = ''
          services.dnscrypt-proxy2.enable = true; to use myOptions.dnscrypt.blocklist
        '';
      }
      {
        assertion =
          config.myOptions.dnscrypt.cloakingRules == '''' || config.services.dnscrypt-proxy2.enable;
        message = ''
          services.dnscrypt-proxy2.enable = true; to use myOptions.dnscrypt.cloakingRules
        '';
      }
    ];
    ...
    services.dnscrypt-proxy2 = {
      ...
      settings = {
        sources.public-resolvers = {
          ...
          cache_file = "/var/lib/${config.myOptions.dnscrypt.stateDirectory}/public-resolvers.md";
        };
        ...
        blocked_names.blocked_names_file = pkgs.writeText "blocklist.txt" config.myOptions.dnscrypt.blocklist;
        cloaking_rules = pkgs.writeText "cloaking_rules.txt" config.myOptions.dnscrypt.cloakingRules;
      };
    };

    systemd.services.dnscrypt-proxy2.serviceConfig.StateDirectory =
      config.myOptions.dnscrypt.stateDirectory;
  };
}
